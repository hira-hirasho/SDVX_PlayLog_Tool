...
SCOREを取得できた場合、`play_log.min_score`以上であれば保存対象とする。

SCOREを取得できなかった場合は、SCOREの判定を行わず保存対象とする。

SCOREを取得できたうえで`play_log.min_score`未満の場合は、プレイ記録およびメディアを保存しない。
...
SCOREは画面上で表示される4桁単位の2つの領域を個別にOCRし、左側の値と右側の値を連結して8桁のSCOREとして扱う。

SCOREのいずれか一方の領域を正常に認識できない場合、SCORE全体を欠損値として扱う。
...
通常の自動記録では、SCOREを取得できた場合は`play_log.min_score`以上であれば保存対象とする。

SCOREを取得できなかった場合は、SCOREによる保存条件を判定せず、自動記録の保存対象とする。

SCORE以外の項目が取得できなかった場合も、その項目を欠損値として保存する。
...
SCOREを取得できたうえで`play_log.min_score`未満の場合は保存しない。
...
通常リザルトでは、SCOREを取得できた場合は`play_log.min_score`以上であれば保存する。

SCOREを取得できなかった場合は、SCOREによる保存条件を判定せず保存対象とする。

SCORE以外の項目が取得できなかった場合も、該当項目を欠損値として保存する。
...
SCOREが取得でき、かつ`play_log.min_score`未満の場合は、記録およびメディア保存を行わない。

SCORE以外のOCR項目が欠損している場合でも、保存条件を満たしていればプレイ記録を保存する。
