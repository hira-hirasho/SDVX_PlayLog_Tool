...
曲開始検出は、リザルト画面に入るまで継続する。

曲開始を検出した時刻を保持し、同じプレイ中に再度曲開始を検出した場合は、検出時刻を最新の値へ更新する。

これにより、リトライ等によって曲開始が複数回発生した場合でも、最後に検出した曲開始時刻をReplay動画のトリミング基準として使用する。

曲開始検出後も曲開始監視は停止しない。

リザルト画面への進入を検出した場合、曲開始監視を停止する。

曲開始を検出できなかった場合でも、F12による手動記録は可能とする。

### 11.2 リザルト画面検出

リザルト画面の検出には、設定された検出ROIだけを直接画面からキャプチャして使用する。

リザルト画面が表示されていない状態でリザルト画面を初めて検出した場合、その検出直後にフルスクリーンを1回キャプチャし、その画像から結果画像を生成・保持する。

リザルト画面の表示中は、検出ROIのみを継続してキャプチャしてリザルト画面の表示状態を監視する。表示中の監視ではフルスクリーンキャプチャを行わない。

リザルト画面が3回連続して未検出となった場合、リザルト画面の終了と判定する。

リザルト画面の終了時には新たな結果画像を取得せず、リザルト画面への進入時に取得した結果画像をそのプレイの結果画像として使用する。

リザルト画面の終了後、リザルト監視を終了し、曲開始監視を再開する。
...
曲開始が検出されている場合、以下の値を使用してReplay動画の冒頭トリミング開始位置を算出する。
...
曲開始検出はリザルト画面に入るまで継続し、複数回検出された場合は最後に検出した時刻を使用する。

取得したReplay動画の実際の動画時間を取得し、以下の計算によって冒頭からのトリミング開始位置を求める。
...
この位置より前をトリミングすることで、加工後のReplay動画の先頭が最後に検出した曲開始時点となるようにする。

曲開始を検出できていない場合は、冒頭トリミングを行わずReplay動画全体を保存する。

トリミング開始位置が0秒以下となった場合も、冒頭トリミングを行わずReplay動画全体を保存する。
...
結果画像は、リザルト画面への初回進入検出直後に取得したフルスクリーンキャプチャから生成する。

リザルト画面の検出には検出ROIを使用するが、結果画像には検出ROIではなくフルスクリーンキャプチャを使用する。

リザルト画面の表示中は結果画像を再取得しない。

リザルト画面終了時にも結果画像を再取得しない。
...
曲開始検出では、設定された曲開始検出領域を使用する。

リザルト画面検出では、設定されたリザルト検出ROIだけを直接キャプチャして検出に使用する。検出のたびにフルスクリーンを取得してからROIを切り出す方式は使用しない。

リザルト画面への進入を検出した場合のみ、結果画像生成用としてフルスクリーンを1回キャプチャする。
...
SCOREを取得できなかった場合も、自動記録対象として扱い、上記と同じ保存処理を実行する。
