...
画面キャプチャの取得責務は`ScreenCapture`および`ResultDetectionCapture`へ分離する。
...
リザルト画面の検出では、設定された検出ROIだけを直接キャプチャする。

リザルト画面への初回進入を検出した場合のみ、結果画像生成用としてフルスクリーンを1回キャプチャする。

検出のたびにフルスクリーンを取得してからROIを切り出す方式は使用しない。
...
リザルト検出では、リザルト検出用ROIを直接キャプチャして処理する。
...
テンプレートは90度反時計回りに回転した状態で管理する。

検出時は、テンプレートと取得したROIを同一スケールへ縮小してテンプレート照合を行う。
...
ResultScreenDetectorは画像取得を担当せず、入力された検出ROIに対してテンプレート照合のみを行う。
...
リザルト画面への初回進入を検出した直後にフルスクリーンを1回キャプチャし、その画像をResultStateの基準画像とする。

リザルト検出に使用したROI画像を結果画像として使用しない。
...
リザルト画面の表示中は、結果画像生成用のフルスクリーンを再キャプチャしない。

リザルト画面終了時にも再キャプチャしない。
...
SongStartMonitorは曲開始を1回検出した時点で停止せず、リザルト画面が検出されるまで監視を継続する。

曲開始が複数回検出された場合は、最後に検出した時刻を最新のSongStart時刻としてMain側へ通知する。
...
Main側では最後に通知されたSongStart検出時刻を保持する。
...
SongStartMonitorがリザルト画面に入るまで監視を継続するため、複数回検出された場合は最後に検出した時刻を使用する。
...
SongStart検出時刻がF12受信時刻より後になる等、経過時間が0未満となる場合は無効値として扱う。
...
`result.png`はResultStateが保持する、リザルト画面への初回進入検出直後に取得したフルスクリーン由来のリザルト画像から生成する。

ResultState保持後の再キャプチャは行わない。
...
SongStartMonitorは曲開始を検出した後も停止せず、リザルト画面の初回検出まで監視を継続する。

複数回のSongStart検出が発生した場合、最後に検出した時刻をReplayトリミング計算に使用する。

ResultMonitorはリザルト画面への初回進入時のみフルスクリーンを取得し、その画像をResultStateが保持する。

リザルト画面表示中の検出では、リザルト検出ROIのみを直接取得する。
